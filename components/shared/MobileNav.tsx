...
const MobileNav = () => {
  ...
  return (
    <Sheet>
      ...
      <SheetContent
        side={"right"}
        className="flex flex-col pt-10 justify-between overflow-auto"
      >
          ...
          {navbarLinks.map((link) => {
            ...
            return (
              <SheetClose asChild key={link.name}>
                <Link
                  href={link.path}
                  className={`flex justify-start items-center gap-4 font-semibold text-2xl rounded-lg w-fit p-[5px] ${active}`}
                >
                  {link.name}
                </Link>
              </SheetClose>
